...
export default function PartnerDashboard() {
  ...
  if (!location) return <div className="min-h-[100dvh] flex items-center justify-center bg-red-50 p-6 text-center"><div className="flex flex-col items-center"><AlertCircle className="w-16 h-16 text-red-500 mb-4" /><h1 className="text-2xl font-black text-black">Podnik nebol nájdený</h1></div></div>;

  if (!isAuthenticated) {
    return (
      <main className="min-h-[100dvh] bg-gray-50 flex flex-col items-center justify-center p-4 md:p-6 font-sans">
        <div className="w-full max-w-sm bg-white p-8 rounded-[2rem] shadow-xl border-2 border-gray-100 text-center flex flex-col items-center">
          ...
          <form onSubmit={handleLogin} className="w-full">
            <input 
              ...
              placeholder="••••••" 
              className="w-full text-center text-4xl tracking-[0.5em] font-mono font-black py-4 bg-gray-50 border-2 border-gray-200 rounded-2xl mb-4 outline-none focus:border-black" 
            />
            {loginError && <p className="text-red-500 font-bold text-xs mb-4 uppercase">{loginError}</p>}
            <button type="submit" disabled={pinInput.length !== 6} className="w-full bg-black text-white font-black py-5 rounded-2xl active:scale-95 transition-all shadow-xl shadow-black/10 disabled:opacity-30">
              Vstúpiť
            </button>
          </form>
        </div>
        <div className="absolute bottom-6 w-full text-center">
          <p className="text-[10px] font-black text-gray-400 tracking-[0.2em] uppercase">Powered by Docenta</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="relative h-[100dvh] w-full bg-gray-100 flex flex-col overflow-hidden font-sans text-black">
      ...
      <header className="p-5 md:p-6 bg-white border-b-2 border-gray-100 flex items-center justify-between shrink-0">
        <div>
          ...
          <h1 className="text-lg font-black text-black tracking-tight">{location.name}</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 pb-32 flex flex-col items-center">
        {booking ? (
          <div className="animate-in slide-in-from-bottom-8 duration-300 w-full max-w-md">
            <div className="bg-white rounded-[2.5rem] p-6 md:p-8 shadow-2xl border-2 border-gray-100 mt-4">
              ...
              <div className="flex justify-between items-start mb-8 border-b-2 border-gray-100 pb-6">
                <div>
                  ...
                  <h2 className="text-3xl md:text-4xl font-black text-black tracking-tight font-mono">{booking.bookingId}</h2>
                </div>
              ...
              <div className="space-y-6 mb-8 text-sm md:text-base">
                ...
                <div className="flex justify-between items-center"><span className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Kontakt</span>
                  {booking.userPhone ? (
                    <a href={`tel:${booking.userPhone}`} className="font-black text-blue-600 text-base flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-xl border border-blue-100"><Phone className="w-4 h-4" /> Volať</a>
                ...
                <div className="pt-4 border-t-2 border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {booking.items?.map((item: any, i: number) => (
                      <span key={i} className="bg-gray-50 border-2 border-gray-200 text-black text-[10px] font-black px-3 py-2 rounded-xl flex items-center gap-2 uppercase tracking-wider"><Package className="w-3 h-3 text-gray-400" /> {item.label}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              {booking.status === 'COMPLETED' ? (
                ...
                <button 
                  ...
                  className={`w-full font-black text-base md:text-lg py-5 rounded-[1.5rem] shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-transform uppercase tracking-widest ${booking.status === 'PENDING' ? 'bg-black text-white' : 'bg-blue-600 text-white'}`}
                >
                  {isLoading ? <Loader2 className="animate-spin w-7 h-7" /> : (booking.status === 'PENDING' ? <><Check className="w-6 h-6 md:w-7 md:h-7" /> Prevziať</> : <><ArrowRight className="w-6 h-6 md:w-7 md:h-7" /> Odovzdať</>)}
                </button>
              )}
          ...
          <div className="max-w-sm mx-auto w-full">
            {activeTab === 'scan' ? (
              <div className="animate-in fade-in duration-500 w-full">
                <button onClick={() => setIsScanning(true)} className="w-full bg-black text-white py-12 md:py-16 rounded-[2.5rem] shadow-2xl shadow-black/20 flex flex-col items-center justify-center gap-6 active:scale-95 transition-transform mb-10">
                  <ScanLine className="w-16 h-16 md:w-20 md:h-20" />
                  <span className="text-xl md:text-2xl font-black uppercase tracking-widest">Skenovať QR</span>
                </button>
                ...
                <div className="bg-white p-2 rounded-[2rem] flex border-2 border-gray-200 focus-within:border-black transition-all shadow-sm">
                  <input type="text" value={manualCode} onChange={(e) => setManualCode(e.target.value.toUpperCase())} placeholder="Zadajte 6 znakov" className="flex-1 bg-transparent px-4 py-3 md:py-4 outline-none font-black text-lg md:text-xl tracking-[0.3em] text-black w-full min-w-0" maxLength={6} />
                  <button onClick={() => handleSearch(manualCode)} disabled={manualCode.length < 6 || isLoading} className="bg-black text-white px-5 md:px-6 rounded-[1.5rem] disabled:opacity-20 flex items-center justify-center shrink-0">
              ...
              <div className="animate-in fade-in duration-500 w-full">
                <h3 className="text-xl font-black mb-6 text-center md:text-left">Aktívne rezervácie</h3>
                      ...
                      <button key={b.id} onClick={() => setBooking(b)} className="w-full bg-white p-5 rounded-3xl shadow-lg border-2 border-gray-100 flex flex-col active:scale-95 transition-transform text-left group">
                        <div className="flex justify-between items-start w-full mb-3 gap-2">
                          <div className="overflow-hidden">
                            ...
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">{b.items?.length || 0} kusy • {b.bookingId}</p>
                          </div>
                          <div className="bg-gray-50 p-3 rounded-2xl group-hover:bg-black group-hover:text-white transition-colors shrink-0"><ArrowRight className="w-4 h-4 md:w-5 md:h-5" /></div>
                        </div>
                        <div className="pt-3 border-t-2 border-gray-50 w-full flex justify-between items-center">
      ...
      {/* SPODNÁ NAVIGÁCIA */}
      <div className="absolute bottom-0 w-full bg-white border-t-2 border-gray-100 p-4 pb-8 flex gap-4 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] justify-center">
        <button 
          onClick={() => { setBooking(null); setActiveTab('scan'); }} 
          className={`w-full max-w-[180px] flex flex-col items-center justify-center gap-1.5 py-3 rounded-2xl transition-all ${activeTab === 'scan' ? 'bg-black text-white shadow-xl shadow-black/20' : 'text-gray-400 hover:bg-gray-50'}`}
        >
        ...
        <button 
          ...
          className={`w-full max-w-[180px] flex flex-col items-center justify-center gap-1.5 py-3 rounded-2xl transition-all ${activeTab === 'list' ? 'bg-black text-white shadow-xl shadow-black/20' : 'text-gray-400 hover:bg-gray-50'}`}
        >
